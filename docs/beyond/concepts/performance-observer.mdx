...
Core Web Vitals are Google's essential metrics for user experience. Sites meeting all three Core Web Vitals thresholds see significantly fewer page abandonment rates. Performance Observer is how you measure them in the field.
...
<AccordionGroup>
  ...
  <Accordion title="What is a Long Task and why does it matter?">
    A Long Task is any JavaScript task that blocks the main thread for more than 50ms. During a Long Task, users cannot click, scroll, or type. Long Tasks are a primary cause of poor INP scores. Monitor them with `observer.observe({ type: 'longtask' })` and break up heavy code into smaller chunks.
  </Accordion>
</AccordionGroup>
