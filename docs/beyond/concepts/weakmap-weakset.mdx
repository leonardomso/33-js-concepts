...
As of ES2023, WeakMap and WeakSet can also hold non-registered [Symbols](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Symbol). This change was made because non-registered Symbols have the same uniqueness and garbage-collectible properties as objects, making them natural candidates for weak references:
...
<AccordionGroup>
  <Accordion title="What is a WeakMap in JavaScript?">
    A WeakMap is a collection of key-value pairs where keys must be objects and are held via weak references. When no other strong references to a key object remain, the entry is automatically removed by garbage collection. WeakMap is ideal for attaching metadata to objects without preventing their cleanup.
  </Accordion>
