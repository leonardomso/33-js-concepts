...
Event delegation significantly reduces memory usage when dealing with many elements. Minimizing the number of event listeners is a key strategy for improving Interaction to Next Paint (INP) scores:
...
<AccordionGroup>
  ...
  <Accordion title="When should I use event delegation?">
    Use delegation when you have many similar elements that need the same handler, when elements are added or removed dynamically, or when memory efficiency matters. Reducing the number of event listeners directly improves page interactivity and INP scores.
  </Accordion>
