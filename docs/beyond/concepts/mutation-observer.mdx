...
How do you know when something changes in the DOM? You might need to react when a third-party script adds elements, when user input modifies content, or when attributes change dynamically.
...
The **[MutationObserver](https://developer.mozilla.org/en-US/docs/Web/API/MutationObserver)** API lets you watch the DOM for changes and react to them efficiently. It replaced older Mutation Events and is now the standard way to detect DOM modifications.
...
MutationObserver solves these problems by batching changes and delivering them asynchronously via microtasks. The redesign was driven by real-world performance issues caused by Mutation Events in complex web applications.
...
MutationObserver callbacks are scheduled as **microtasks**, so they run after the current script and before the browser renders. This batching behavior is intentional: it lets multiple DOM changes be processed in a single callback invocation. This is the same queue used by Promise callbacks.
...
<AccordionGroup>
  ...
  <Accordion title="When do MutationObserver callbacks execute?">
    Callbacks run as microtasks — after the current synchronous script finishes but before the browser paints. Multiple rapid DOM changes are batched into a single callback invocation, which is why MutationObserver is far more efficient than the synchronous Mutation Events it replaced.
  </Accordion>
</AccordionGroup>
