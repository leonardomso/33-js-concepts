...
The **[`this`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/this)** keyword is one of JavaScript's most confusing features, but it follows specific rules. Once you understand them, you'll never be confused again.
...
<Note>
Arrow functions are listed last not because they're lowest priority, but because they work differently. Arrow functions do not have their own `this` binding at all — they inherit it from the enclosing lexical scope. We'll cover them in detail.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Why do arrow functions not have their own 'this'?">
    Arrow functions were designed to solve the common problem of losing `this` context in callbacks. Arrow functions do not have their own `this` binding — they inherit `this` from their enclosing lexical scope. This makes them ideal for callbacks and event handlers where you want to preserve the outer `this`.
  </Accordion>
