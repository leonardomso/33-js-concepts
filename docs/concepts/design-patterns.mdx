...
The **Singleton pattern** ensures a class has only one instance and provides a global access point to that instance. It solves two problems: guaranteeing a single instance and providing global access to it.
...
The **Observer pattern** defines a subscription mechanism that notifies multiple objects about events. It lets you notify multiple objects about events that happen to the object they are observing.
...
The **Decorator pattern** attaches new behaviors to objects by wrapping them in objects that contain these behaviors. It lets you add behavior by placing objects inside special wrapper objects.
