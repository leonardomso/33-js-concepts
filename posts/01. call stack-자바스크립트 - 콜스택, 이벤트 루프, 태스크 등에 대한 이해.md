...
console.log()는 빠르겠지만 for나 while로 수천 또는 수백만 라인의 항목을 수행하는 반복문은 느려질 것이며, 스택을 점유하거나 블러킹할 것입니다.
이것은 블로킹 스크립트(blocking script)로 불리우며, 당신은 웹페이지 스피드 인사이트에서 그에 대해 읽어나 들었을 것입니다.

네트워크 요청은 느려질 수 있고 이미지 요처도 느려질 수 있습니다. 하지만 고맙게도 서버 요청은 비동기 함수인 AJAX를 통해 수행될 수 있습니다. 이 네트워크 요청이 동기 함수를 통해 만들어진다 가정할때 어떤 일이 발생할까요?
네트워크 요청은 기본적으로 어딘가에 있는 다른 서버로 발송이 됩니다. 이제 컴퓨터는 응답을 느리게 되돌릴 수 있습니다.
